...
class Map extends Component {
    ...
    componentDidUpdate(prevProps) {
        ...
        const child = this.map.getContainer().getElementsByClassName("maplibregl-canvas");
        const { width, height } = this.map.getContainer().getBoundingClientRect();
        const childDimension = child?.[0]?.getBoundingClientRect();
        let childDiv = child?.[0];
        if ((Math.abs(width - childDimension?.width) > 0) && childDiv){
